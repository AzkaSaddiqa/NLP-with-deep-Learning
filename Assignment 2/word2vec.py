...
def naiveSoftmaxLossAndGradient(
    ...
    y[outsideWordIdx] = 1
    delta_y_hat = y_hat - y # y_hat - y
    gradCenterVec = np.dot(delta_y_hat, outsideVectors) # dJ / dv_c
    gradOutsideVecs = np.dot(delta_y_hat.reshape(-1, 1), centerWordVec.reshape(1, -1))  # dJ / dU
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_tmp, gradCenterVec_tmp, gradOutsideVecs_tmp = word2vecLossAndGradient(
          centerWordVectors[word2Ind[currentCenterWord]], word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_tmp
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec_tmp
        gradOutsideVectors += gradOutsideVecs_tmp
